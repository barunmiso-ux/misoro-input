...
# 원내 CRM에 문의·초진을 직접 적는 지점 (2026-08-17~ 분당).
# 이 지점은 일일시트 채널칸을 crm_sync.py 가 매일 23:30에 채운다. 앱이 같은 칸을
# 덮어쓰면 0으로 지워지므로, 앱은 **카페숙제만** 기록하고 채널칸은 손대지 않는다.
CRM_BRANCHES = ["분당"]
...
ROUTE_GROUPS = {
    "온라인": ["홈페이지", "네이버 블로그", "네이버 카페", "네이버 지식인", "네이버(미상)", "뉴스기사",
             "지도", "배너", "유튜브", "SNS", "페이스북", "인스타", "밴드", "카톡", "모름"],
    "오프라인": ["방송(TV)", "라디오", "소개", "강의", "간판", "협약기관", "버스광고",
              "지하철광고", "택시광고", "신문광고", "현수막", "기타"],
    # 내부이동: 마케팅 유입 아님. 전환율 분모/분자에서 제외(매출은 OKTAS결산에 자동포함).
    "내부이동": ["타지점이관"],
}
...
# ── 카페숙제체크 탭 ──────────────────────────────────────────────
HOMEWORK_TAB = "카페숙제체크"
# 위치 보존: 기존 C/D/E(건강카페·건강홈페·일상게시글) = 신 칼럼카페·칼럼홈페·게시글
# (라벨만 변경, 데이터 위치 동일). 상담답변만 J열로 신규 추가.
HOMEWORK_HEADERS = [
    "날짜", "지점", "칼럼_카페", "칼럼_홈페", "게시글",
    "댓글수", "댓글URL", "입력자", "기록시각", "상담답변",
]
