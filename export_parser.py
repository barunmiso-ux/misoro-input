...
- 차트의 재활용 칸 해석: EMail=결제여부, 직업=상담자, 진행치료=한약/치료, URL=비예약원인.
...
class Patient:
    ...
    inflow: str = ""        # 유입경로
    counselor: str = ""     # 상담자
    booking: str = ""       # 결제/결제안함/미상 (EMail 칸)
...
def _parse_booking(email_cell: str) -> str:
    ...
    # 예약/예약안함 = 결제여부와 무관(워크인도 결제함) → 절대 변환 안 함, 미상 처리해 앱이 차단.
    if "예약" in v:
        return "미상"
    # 결제 안 함 먼저 — '미결제'처럼 '결제'를 포함해도 안함으로 잡히게
    if any(k in v for k in ("결제안함", "미결제", "미수납", "안결제")):
        return "결제안함"
    if "결제" in v:
        return "결제"
    return "미상"
...
def parse_export(path: str) -> dict:
    ...
    booked = sum(1 for p in patients if p.booking == "결제")
    ...
    return {
        ...
        "summary": {
            "초진수": len(patients),
            "질환군별": grp_counts,
            "결제": booked, "결제안함": sum(1 for p in patients if p.booking == "결제안함"),
